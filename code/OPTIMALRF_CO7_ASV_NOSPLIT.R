...
# importance.df <- read.table("RFEVarImp_Class-log.μgCO2.7.norm_ASV_AllRepTrain.tsv", header = TRUE, sep = "\t")
...
p <- ggplot(importance.df, aes(x = mean_min_depth, y = node_purity_increase)) +
  ...
  ggtitle("F) Carbon dioxide Production") +  # Add the plot title
  theme_minimal() +
  theme(
...
pdf("Plot_RFEMultiwayImp_CO_ASV2.pdf", height =6)
print(p)
