...
# ONLY INCLUDING CLUSTERS OF SIZE 2 OR MORE
...
p <- ggplot(importance.df, aes(x = mean_min_depth, y = node_purity_increase)) +
  ...
  ggtitle("Explaining variation in the ATP concentration per cell between microbial community samples using the abundances of subgroups") +  # Set the plot title dynamically
  theme_minimal() +
  theme(
...
pdf("Plot_RFEMultiwayImp_ATP_CLUSTER.pdf", height =6)
print(p)
