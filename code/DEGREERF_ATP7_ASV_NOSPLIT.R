...
# importance.df <- read.table("DEGREERFEVarImp_Class-log.ATP7.norm_ASV_AllRepTrain.tsv", header = TRUE, sep = "\t")
...
p <- ggplot(importance.df, aes(x = mean_min_depth, y = node_purity_increase)) +
  ...
  ggtitle("Importance of taxa in explaining differences\nin ATP production between microbial community samples") +  # Set the plot title dynamically
  theme_minimal() +
  theme(
...
pdf("DEGREEPlot_RFEMultiwayImp_ATP_ASV.pdf", width = 7, height = 6)
print(p)
