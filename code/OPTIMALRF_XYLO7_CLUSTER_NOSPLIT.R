...
p <- ggplot(importance.df, aes(x = mean_min_depth, y = node_purity_increase)) +
  ...
  ggtitle("Explaining variation in the xylosidase concentration per cell between microbial community samples using the abundances of subgroups") +  # Set the plot title dynamically
  theme_minimal() +
  theme(
...
pdf("Plot_RFEMultiwayImp_XYLO_CLUSTER.pdf", height =6)
print(p)
