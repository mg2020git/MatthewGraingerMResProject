...
# Parameters for RF
mtry.min="default" # number of variables randomly selected to build the trees. Fix to "default" if you don't have an informed guess
...
# Numbers of trees to test (12 different numbers in total)
ntree.test=c(100,250,500, seq(from=1000,to=11000,by=2000),14000,18000,22000)

# Function that takes a number of trees, and performs RF using the data set with fixed number of variables
random_forest_treenum <- function(ntree_input){ # yIn, xIn, and mtry.test are already defined so they do not need to be inputs for the function
  RF.tmp <- randomForest(y=yIn,x=xIn,
                         importance=T, proximity = T, 
                         ntree=ntree_input, mtry = mtry.test)
...
ntree.tmp <- ntree.test[iter] # The tree for the current job e.g. job 1 will select first tree number
  
# Vector that stores results
mse_results <- numeric(Nrand)

# 
#mse_results <- random_forest_treenum(ntree_input = ntree.tmp)

# Parallel backend
num_cores <- 4 # CHANGE DEPENDING ON NUMBER OF CPUS ALLOCATED IN PBS SHELL SCRIPT
...
mse_results <- foreach(i=1:Nrand, .combine='c', .packages='randomForest') %dopar% {
  random_forest_treenum(ntree.tmp)
}
...
mse_results <- c(ntree.tmp, mse_results)
saveRDS(mse_results, file=paste0("test_mses_", ntree.tmp, "_trees.rds"))
# This file contains the results vector.
# The first item is the number of trees.
# Each of the other items is an MSE value for one of the 25 test RFs of that tree number
